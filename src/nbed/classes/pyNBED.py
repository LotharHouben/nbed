...
class pyNBED:
    ...
    def __filterpeaklist(self, framepeaklist,refine=False, massthresh=None):
        """ filter a list of plot of peak locations from a framepeaklist 
        ...
        for f in framepeaklist:
            if (f['x'][1:]).size:
                ...
                if refine:
                    ...
                    y0=f['y'][0]   
                if massthresh:
                    filt=np.argwhere((f['raw_mass'][1:]>= massthresh)) 
                    if filt.any():
    ...
    def DebyeScherrerPlot(self, framepeaklist,refine=False, massthresh=None):
        ...
        """
        (x,y)=self.__filterpeaklist(framepeaklist, refine=refine,massthresh=massthresh)
        N=x.size
        colors = np.random.rand(N)
        plt.figure()
        plt.gca().set_aspect('equal')
        scat=plt.scatter(x, y, s=2,c=colors)
        plt.show
        return (x,y)

    def PeakDistanceHistogram(self, framepeaklist, refine=True, massthresh=None):
    ...
    def VirtualApertureImage(self, radius=[0,10], offset=None, invert=False):
